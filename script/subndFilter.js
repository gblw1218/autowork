/**
 * 过滤节点列表，根据 IP 和端口移除重复项。
 * @param {Array<Object>} nodes - 节点对象数组，每个对象都应包含 'host' 和 'port' 属性。
 * @returns {Array<Object>} 一个只包含唯一节点的新数组。
 */
function filterNodes(nodes) {
    ...
    for (const node of nodes) {
        // 提取 IP 和端口，创建唯一的 IP:Port 字符串
        const ipPort = `${node.host}:${node.port}`;

        // 如果 IP:Port 组合未见过，则将其添加到结果列表并记录
        if (!seen.has(ipPort)) {
            seen.add(ipPort);
...
/**
 * 定制和过滤 Shadowrocket 代理节点名称的函数。
 * 此函数旨在 Shadowrocket 的脚本环境中运行，对传递给它的单个节点进行操作。
 *
 * @param {Object} $server - Shadowrocket 正在处理的当前服务器（节点）对象。
 * 它必须有一个 'title' 属性。
 * @returns {boolean} - 如果节点应被保留并更新其标题，则返回 true；如果应被丢弃，则返回 false。
 */
function processNodeTitle($server) {
    // --- 自定义前缀和后缀 ---
    let customCharStart = "王大大-"; // 添加前缀，删除引号内的内容即不添加前缀
    let customCharEnd = ""; // 添加后缀，删除引号中的内容即不添加后缀
    const outputLanguage = "CN"; // 选择 "CN" 或 "EN" 作为输出语言

    // --- 关键词到名称的映射 ---
    // 根据节点标题中的关键词，将其映射为标准化且带有表情符号的名称
    ...
    // --- 过滤关键词列表 ---
    // 如果节点标题包含这些关键词，则认为其为无效或广告节点，将被过滤掉
    ...
    // --- 保留并替换的关键词映射 ---
    // 定义需要保留并可能进行替换的关键词，它们会重新加回标题的末尾
    const keywordsMap = {
        "ChatGPT": "GPT",
        "解锁": "解",
        "中转": "中转",
    ...
    // 检查是否包含过滤关键词。如果包含，则丢弃该节点（返回 false）
    if (filterKeywords.some(kw => new RegExp(kw, 'i').test($server.title))) {
        return false;
    }
    ...
    // 遍历 keywordsMap，提取并移除标题中匹配的关键词
    for (const kw in keywordsMap) {
        let match = newTitle.match(new RegExp(kw, 'i'));
        if (match) {
            preservedParts.push(keywordsMap[kw]); // 将替换词添加到保留部分
            newTitle = newTitle.replace(match[0], ''); // 从标题中移除已匹配的关键词部分
        }
    }

    // --- 匹配地区关键词并替换标题 ---
    let titleFlag = false; // 标记是否成功匹配到地区关键词
    // 遍历 keywordsToNames，尝试匹配地区关键词并替换节点名称
    for (const keyword in keywordsToNames) {
        if (new RegExp(keyword, 'i').test(newTitle)) {
            newTitle = keywordsToNames[keyword]; // 使用对应的标准化名称
            titleFlag = true; // 标记为已匹配
            break; // 找到第一个匹配项后即停止
        }
    }

    // --- 处理未匹配到地区关键词的节点 ---
    // 如果没有匹配到任何地区关键词，则丢弃该节点（返回 false）
    if (!titleFlag) {
        return false;
    }
    ...
    // --- 防止节点标题重复 ---
    // 使用一个全局对象 (map) 来跟踪已生成的标题，以避免重复
    // 假设 Shadowrocket 的脚本环境允许全局变量在不同节点处理之间持久存在。
    const map = globalThis.map || (globalThis.map = {});

    if (!map[newTitle]) {
        map[newTitle] = 1; // 如果标题是第一次出现，则初始化计数
    } else {
        newTitle = `${newTitle}-${++map[newTitle]}`; // 如果标题已存在，则添加递增数字后缀
    }
    ...
    if (preservedParts.length) {
        newTitle += ' ' + preservedParts.join(' '); // 将保留部分以空格连接并添加到标题末尾
    }
